...
def upgrade() -> None:
    try:
        # Try JSON for broader compatibility
        op.add_column('hosts', sa.Column('ss2022_relay_inbound_tags', sa.JSON(none_as_null=True), nullable=True))
    except Exception:
        # Fallback to String for legacy engines
